...
class ModulationDomainLossModule(torch.nn.Module):
    ...
    def __init__(self, modulation_kernels, norm=False):
...
class ModulationDomainNCCLossModule(torch.nn.Module):
    ...
    def forward(self, enhanced_spect, clean_spect):
        ...
        normalized_denom = (torch.sum(normalized_clean * normalized_clean, dim=2)
                            )**.5 * (torch.sum(normalized_enhanced * normalized_enhanced, dim=2))**.5

        ncc = inner_product / normalized_denom
        mod_mse_loss = torch.mean((ncc-1.0)**2)
        #mod_mse_loss = -torch.mean(ncc)
...
if __name__ == '__main__':
    ...
    # compute modulation loss value used for training
